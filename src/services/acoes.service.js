...
// Criar uma ação nova
exports.criar = async ({ cliente_id, designado_id, titulo, status, criador_id, arquivos }) => {
    // Buscar cliente
    const [cliente] = await executeQuery('SELECT nome, cpf_cnpj FROM cliente WHERE id = ?', [cliente_id]);
    if (!cliente) throw new Error('Cliente não encontrado');

    // Buscar designado (se não for “Nenhum”)
    let designadoNome = null;
    if (designado_id && designado_id !== 'Nenhum') {
        const [d] = await executeQuery('SELECT nome FROM usuarios WHERE id = ?', [designado_id]);
        if (d) designadoNome = d.nome;
    }
    ...
    for (const arquivo of arquivos) {
        ...
        const destino = path.join(pastaAcao, prefixo + arquivo.originalname);
    ...
    const result = await executeQuery(
        ...
        [cliente.nome, titulo, designadoNome, criadorNome, status, pastaAcao]
    );

    return { id: result.insertId, pasta: pastaAcao };
};

// Upload adicional (ação, contrato, doc, provas)
...
exports.getStatus = async (id) => {
    const [r] = await executeQuery('SELECT status FROM acoes WHERE id = ?', [id]);
    return r ? r.status : null;
};

// Atualizar status/designado
exports.updateStatus = (id, { status, designado }) => {
    if (status === 'finalizado') {
        return executeQuery('UPDATE acoes SET status = ?, designado = ?, data_concluido = NOW() WHERE id = ?', [status, designado, id]);
    } else if (status === 'em andamento') {
        return executeQuery('UPDATE acoes SET status = ?, designado = ?, data_concluido = NULL WHERE id = ?', [status, designado, id]);
    }
    return executeQuery('UPDATE acoes SET status = ?, designado = ? WHERE id = ?', [status, designado, id]);
};
...
exports.listarArquivos = async (id) => {
    ...
    if (!acao || !acao.arquivo_path || !(await fs.pathExists(acao.arquivo_path))) {
        return { Contrato: [], Documentacao: [], Provas: [], Acao: [], __designadoAtual: acao?.designado || 'Nenhum' };
    }
